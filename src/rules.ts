...
const totalAmount = (value: number, arr: number[]): number => {
  return value * count(value, arr);
};

const threeOfKind = (arr: number[]): number => {
  const freq = frequency(arr);
  return freq.some((item) => item >= 3) ? sum(arr) : 0;
};

const fourOfKind = (arr: number[]): number => {
  const freq = frequency(arr);
  return freq.some((item) => item >= 4) ? sum(arr) : 0;
};

const fullHouse = (arr: number[]): number => {
  const freq = frequency(arr);
  return freq.includes(3) && freq.includes(2) ? 25 : 0;
};

const smallStraight = (arr: number[]): number => {
  const firstCase =
...
const largeStraight = (arr: number[]): number => {
